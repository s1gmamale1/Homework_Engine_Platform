"""Convert a lesson markdown file to properly formatted .docx."""
import re
import sys
import os
...
INPUT = os.path.join(
    ...
    "..", "standards", "standards_specifications", "grades", "5", "uz", "science",
    "lessons", "lesson-01-ch1.1-gul.md"
)
...
def add_fmt(doc, text, st="Normal"):
...
def shade(cell, color):
    el = parse_xml(f'<w:shd {nsdecls("w")} w:fill="{color}"/>')
...
while i < len(lines):
    ...
    if line.strip().startswith("```"):
        if not in_code:
            ...
            in_code = False
            for cl in code_buf:
                doc.add_paragraph(cl, style="CodeBlock")
        i += 1
        continue
