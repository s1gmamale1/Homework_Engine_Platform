...
**Buzan Injection — Phase 5 is Buzan's home phase.** This is where most Buzan mnemonic methods live. The technique is selected based on the CONTENT STRUCTURE, not randomly:
...
|---|---|---|
| Science | Radiant Summary (classification), Link System (processes), Memory Palace (anatomy) | Major System (few numbers) |
| Math | Peg System (formula names), Radiant Summary (formula families) | Link System (math isn't sequential narrative), Memory Palace (abstract) |
| History | Link System (timelines), Major System (dates), Memory Palace (locations) | Radiant Summary (history is temporal, not hierarchical) |
| Geography | Memory Palace (locations on map), Radiant Summary (biome categories) | Major System, Link System |
| Tarbiya | Radiant Summary (virtue categories) | Major System (no numbers), Memory Palace (abstract values don't map to locations well) |
...
| Language | Peg System (vocabulary sets), Link System (grammar rule chains) | Major System, Radiant Summary |

**SMASHIN' SCOPE Quality Gate:** Every mnemonic exercise in Phase 5 must score 6+/12 on the SMASHIN' SCOPE checklist — **S**ynaesthesia, **M**ovement, **A**ssociation, **S**ubstitution (concrete for abstract), **H**umor, **I**magination, **N**umber, **S**ymbolism, **C**olor, **O**rder, **P**ositive, **E**xaggeration. Content scoring below 6 is returned for enrichment. This ensures mnemonic imagery is vivid and memorable, not dry.

**Buzan Review Intervals (~10 min touchpoint):** Phase 5 IS the first Buzan review touchpoint (~10 minutes after learning in Phase 2). Concepts learned in Story Mode reappear here in mnemonic form — same content, different encoding channel. This activates Dual Coding (verbal in Phase 2 → visual/spatial in Phase 5).
...
> **Note (2026-04-07):** This catalog now contains **16 mechanics** (Tic Tac Toe vs AI #15, Notebook Capture #16). It is also complemented by a **sibling pool** — the **Interactive Knowledge-Gated Game Catalog** (`standards/NETS-Interactive-Game-Catalog.md`) — which Phase 3 draws from per the dual-catalog selection rule (§5.3, §6.9). The two pools are not redundant: the Default Pool contains the core pedagogy mechanics that map 1:1 to Bloom's/PISA bands; the Interactive Catalog contains classic-game-derived knowledge-gated experiences (Tic Tac Toe is duplicated across both for convenience).
...
**Buzan: Dual Coding.** Pairs MUST be image↔text, not text↔text. If both cards are verbal, only one encoding channel fires. Cross-modal pairs (visual + verbal) double recall probability (Paivio). The game already requires pairs — making them cross-modal costs nothing.
...
**Buzan: Review Intervals.** SM-2 can be too generous with "Easy" rated cards — pushing them weeks out. Buzan's 24h and 1-week touchpoints are critical regardless of perceived ease. Floor constraint: `min(SM-2, Buzan)`. "Easy" cards still return at 24h, 1w, 1mo minimum.
...
**Buzan: 80/20 Rule.** Blanked words should BE the `keywords_80_20` from the narrative segment — the words carrying 80% of meaning. Blanking random words tests random facts; blanking the conceptual anchor trains reading for meaning. Exception: Language grammar exercises where the blank IS a grammatical element.
...
**Buzan: SMASHIN' SCOPE + Review Intervals.** Content placed at hotspots must pass SMASHIN' SCOPE (6+/12) — vivid, multi-sensory imagery. Dry placements perform no better than rote memorization. Palace revisits follow Buzan's schedule (Day 1→3→7→14→30). "Hali emas" framing on missed recalls.
...
**Buzan: TEFCAS.** The adaptive mechanism IS TEFCAS — but the student doesn't see it. On difficulty increase: *"Trial muvaffaqiyatli — keyingi daraja!"* On decrease: *"Feedback olindi — flow zonangizni topmoqdamiz."* Only on TRANSITIONS, not every question.
...
**Buzan: Von Restorff Effect.** The large prize box should contain the "outstanding" question — most surprising, funny, or unusual. Von Restorff means this ONE question is remembered far better than the others. The game's reward structure amplifies a specific piece of knowledge. Don't make ALL questions outstanding — the effect works because ONE stands out.
...
**Buzan: No injection.** Physical activity — no knowledge encoding happening. Buzan methods are cognitive, not physical.
...
**Buzan: Radiant Thinking.** The "Why?" chain IS radiant thinking — each answer branches deeper. Render it as a downward branching tree on screen, not a flat numbered list. Each level = a new branch growing from the previous answer. The student sees their understanding DEEPEN visually. Not for 2-level chains (too short to branch meaningfully).
...
**Buzan: TEFCAS Failure Framing.** Peer Teaching is the only mechanic where students PRODUCE knowledge. "Incorrect" kills the production impulse. Use: *"Hali emas! Tushuntirishingiz [X] ni qamraydi, lekin [Y] yetishmayapti. Qanday Adjust qilasiz?"* — preserves the expert identity. Exception: factually dangerous explanations (safety info) get direct correction.
...
**Buzan: Peg System Integration.** When Peg System is also active in the same session, merge them: cells labeled with peg numbers. Place X on cell 3 = recall concept pegged to number 3. This fuses spatial strategy with mnemonic retrieval — triple coding (visual peg + verbal concept + strategic board position). Only when Peg System is co-selected; otherwise plays normally.
...
**National Pride Injection — 55/45 and 70/30 Merge:** The "Did You Know..." pool is fed by `quotes_database.json` (4,800+ items). Selection respects two balances: 70% facts / 30% quotes (type balance) and 55% national / 45% global (origin balance). All items must be subject-tagged for relevance matching. Examples:
- *National fact:* "Bilarmidingiz? Mirzo Ulug'bek 15-asrda qurilgan observatoriyasi o'sha davrning eng aniq astronomik hisob-kitoblarini amalga oshirgan."
- *Global fact:* "Bilarmidingiz? Dunyodagi eng kuchli superkompyuter 1 soniyada 1 kvintillion hisoblash bajara oladi."
- *National quote:* "Ilm bor joyda taraqqiyot bo'ladi." — Shavkat Mirziyoyev
- *Global quote:* "Tasavvur — bilimdan muhimroqdir." — Albert Einstein
...
### 6.9 Interactive Game Catalog Integration *(NEW 2026-04-07)*

**Catalog File:** `standards/NETS-Interactive-Game-Catalog.md` (v1.0)

**Purpose:** The Interactive Catalog is a complementary pool of 12 knowledge-gated games derived from classic board/card/puzzle mechanics (Tic Tac Toe, Connect Four, Mastermind, Memory, Word Ladder, Sliding Tile, Blackjack, Risk, Escape Room, Bridge Builder, Minesweeper, plus the original Reaction Chain concept). Every game gates board progression behind subject questions: correct answers execute the student's chosen move; wrong answers trigger a strategic penalty (random placement, forced move, lost turn). The goal is to turn recall and application into a duel where strategy amplifies mastery.

**Dual-Catalog Selection Rule (authoritative):**

> When the Phase 3 engine selects the 3 games for a session, it MUST choose **at least 1 game from the Interactive Catalog** AND **at least 2 games from the Default 16-mechanic pool** (§6.1). The Interactive Catalog is a complement, never a replacement.

**Interactive Catalog games (12):**

| # | Name | 1-line description |
|---|------|--------------------|
| 1 | Tic Tac Toe vs AI | Duel minimax AI; correct answer places chosen tile, wrong answer places randomly. |
| 2 | Connect Four vs AI | 7x6 drop grid; correct answer drops in chosen column, wrong answer drops randomly. |
| 3 | Codebreaker | Mastermind-style secret code; each guess attempt gated by a correct answer. |
| 4 | Memory Match Blitz | Concentration grid where matched pairs must be confirmed by a subject question. |
| 5 | Reaction Chain | 6-10 node chain lights up with each correct answer; 3 wrong in a row breaks the chain. |
| 6 | Word Ladder Climb | Transform start word into target word one letter at a time; each step gated by a question. |
| 7 | Puzzle Lock (Sliding Tile) | 3x3/4x4 sliding puzzle where correct answers slide the chosen tile, wrong answers slide a random one. |
| 8 | Blackjack 21 Knowledge Edition | Hit/Stand choices gated by questions; wrong answers let the AI make the choice for you. |
| 9 | Territory Conquest | Simplified Risk-style 1v1 area control; attacks require correct answers, failures lose territory. |
| 10 | Escape Room: Subject Lock | Unlock 4 virtual objects by answering questions; clues cross-hint across objects. |
| 11 | Bridge Builder | Physics bridge construction; each beam placement gated by a question, wrong answers waste budget. |
| 12 | Minefield Navigator | Navigate a hidden-mine grid; correct answers move where you choose, wrong answers force adjacent moves. |

**When to pick a Catalog game vs a Default game:**

- **Pick a Catalog game when** the session needs a high-stakes, strategy-layered duel (recall + application feels dull), when the topic maps cleanly to a spatial/strategic metaphor (chronology → Puzzle Lock, metabolic pathways → Reaction Chain, periodic table → Codebreaker, geography → Territory Conquest), when the student's engagement score has dipped over the last 3 sessions, when the session is a Big Boss or Mythical Boss review (Territory Conquest / Escape Room / Bridge Builder are configured for cross-subject), or when the student has already seen the same Default mechanic more than once this week.
- **Pick a Default game when** the Bloom's target is Remember/Understand and needs maximum repetition density (Spaced Flashcards, Memory Sprint, Tile Match), when the content is narrative-heavy (Story Mode, Why Chain), when the task requires physical work (Notebook Capture), when the student is in a Recovery Session or below PISA L2 (Default mechanics have lower cognitive overhead), or when subject compatibility in the Interactive Catalog's matrix is marked `⚠️` or `❌`.
- **Device / context constraints:** Bridge Builder and Minefield Navigator have higher render cost and should be skipped on low-end devices; Blackjack 21 is math-only; Word Ladder Climb is language/biology only. The engine must fall back to a Default mechanic when a Catalog game fails its validation check — but must still end with ≥1 Catalog game in the final 3.

**Fallback rule:** If no Catalog game passes subject/device validation, the engine logs a `CATALOG_FALLBACK` warning and selects 3 from Default; this is tracked per-student and triggers a content-ops review if it happens more than twice in a week.
