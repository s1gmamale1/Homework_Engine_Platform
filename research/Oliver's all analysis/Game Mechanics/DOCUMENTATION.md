...
7. XP awarded per level completed + completion bonus
...
- XP: 150 per level completed + 250 bonus for completing all levels
- Hint cost: −10 XP per hint used
...
| Criterion | XP | What AI checks |
|---|---|---|
| Completeness | +100 | All key concepts present — no critical gaps |
| Clarity | +100 | Language appropriate for the fictional audience |
| Accuracy | +200 | Factually correct against textbook content |
| **Total** | **400 XP** | Accuracy weighted double — wrong but clear = low score |
...
### Step-by-step flow
1. Session summary shown: stars earned, XP gained, top strength and top weakness from this session
2. Reflection prompt appears (AI-generated based on performance pattern)
3. Student reads the prompt
4. Text area opens — minimum 10 characters required (2 sentences in Extended Mode)
5. Student writes response — no time limit, no AI evaluation of content
6. "Tugatdim" (Finish) button
7. Response stored privately — student-only. Teacher sees themes only, not individual text.
8. Session closure screen (XP chest, streak update, next lesson tease)
...
- XP: none directly — the Reflection Journal is part of the session closure reward sequence
...
→ REFLECTION JOURNAL ←
    ↓
Session Closure (XP chest, streak, next lesson tease)
```
...
**Skippable:** Yes — if the student taps "Skip" the session closes normally with no XP penalty. The Reflection Journal is the only non-mandatory phase element.
...
8. Solved = all tiles in correct order → celebration + XP
9. Time expired without solving = partial XP based on progress
...
- XP: Solved with no wrong answers = 300 XP | Solved with <3 wrong = 200 XP | Solved with 3+ wrong = 100 XP | Not solved = 50 XP
...
The random tile slide creates a "punishment" that is strategic, not punitive — the puzzle becomes harder, but the student can still win. This is the same psychology as the Minefield Navigator: consequences are real but not catastrophic. The student is motivated to answer correctly not just for XP but to maintain control of their strategy.
...
11. XP awarded based on outcome
...
- Win: +200 XP
- Push (tie): +100 XP
- Loss: +50 XP (participation — no zero for losing)
- Correct answers to questions regardless of outcome: +30 XP each
- Perfect game (win without any wrong answers): +100 bonus XP
...
## What It Is
A Wordle-style code-breaking mechanic adapted for education. A secret 4-letter code is hidden. The student has 6 attempts to guess it. After each guess, colour feedback reveals which letters are correct (green = right letter, right position), present (yellow = right letter, wrong position), or absent (gray = not in the code). Before each guess is processed, the student must answer a subject question. Wrong answer = the guess is rejected without colour feedback.

## How It Is Played

### Step-by-step flow
1. 4-letter secret code is set (hidden)
2. Student builds a 4-letter guess using the on-screen keyboard
3. Student taps "Taxmin qilish" (Submit Guess)
4. Question modal appears
5. **Correct answer:** guess is processed — colour feedback displayed
6. **Wrong answer:** guess is discarded (no feedback, attempt not counted)
7. Repeat for up to 6 attempts
8. Solved = celebration + XP scaled by attempts remaining
9. Not solved in 6 = secret code revealed
...
### Parameters
- Code length: 4 letters
- Maximum attempts: 6
- XP: Solved in 1 = 300 XP | 2–3 = 200 XP | 4–6 = 100 XP | Not solved = 30 XP
- Questions: 1 per submitted guess (not per wrong answer)
- On-screen keyboard tracks letter states (green/yellow/gray)
- AI Tier: 1

## Subject Implementations

### Language Sciences (Primary)
- Codes are vocabulary words from the lesson (4-letter Uzbek or English words)
- The process of deducing the word mirrors vocabulary internalization

### Exact Sciences
- Codes are abbreviations: "ATOM", "MASS", "WATT", "KILO"

### Natural Sciences
- Codes: "GENE", "CELL", "LUNG", "ROOT"
...
Real-Life Challenge
    ↓
→ CODEBREAKER — Consolidation ←
    ↓
Boss Fight
```

**Why this position:**
- Requires accumulated vocabulary/knowledge from the full session
- Deductive reasoning matches Bloom's Analyze — appropriate for post-content consolidation
- 6-attempt structure creates natural session arc with rising tension

**Not recommended:**
- Phase 1 (student hasn't encountered vocabulary yet)
- Non-language subjects as primary (letter codes have weak subject connection outside language and abbreviation-heavy sciences)
